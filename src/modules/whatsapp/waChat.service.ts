...
export async function recordIncomingMessage(jid: string, text: string, messageId?: string, pushName?: string) {
  await connectDB()
 ...
 */
export async function backfillHistory(entries: HistoryEntry[], contactNames: Record<string, string>) {
  ...
  const messageOps = entries
    ...
    .map((e) => ({
      updateOne: {
        filter: { tenantId: tenant._id, jid: e.jid, messageId: e.messageId },
        update: { $setOnInsert: { ...e, tenantId: tenant._id, createdAt: e.timestamp } },
        upsert: true,
      },
    }))
  ...
  const existing = await WaContact.find(
    { tenantId: tenant._id, jid: { $in: Array.from(latestByJid.keys()) } },
  ...
  const contactOps = Array.from(latestByJid.entries()).map(([jid, e]) => {
    ...
    return {
      updateOne: {
        filter: { tenantId: tenant._id, jid },
        update: {
          $setOnInsert: { botPaused: false, unreadCount: 0 },
          $set: {
